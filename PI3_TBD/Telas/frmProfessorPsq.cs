...
namespace MVC.Telas
{
    public partial class frmProfessorPsq : Form
    {
        ...
        private void btnConfirmar_Click(object sender, EventArgs e)
        {
            ...
            {
                Professor p = new Professor();
                p.setCod(Convert.ToInt32(txtCodProfessor.Text));
                p.setNome(txtNome.Text);
                p.setEmail(txtEmail.Text);
                p.setIDsenac(txtIdSenac.Text);
                p.setSenha(txtSenha.Text);
                p.setTipo(tipo);
        ...
        private void lsvResultado_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            txtCodProfessor.Text = p.getCod().ToString();
            txtNome.Text = p.getNome();
            txtEmail.Text = p.getEmail();
            txtIdSenac.Text = p.getIDsenac();
            txtSenha.Text = p.getSenha();
            if (p.getTipo() == 'A')
            {
                rbA.Checked = true;
            }
